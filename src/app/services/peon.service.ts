...
export class PeonService {
  ...
  public moverPeon(posInicial: string, posFinal: string){
    ...
    if(ficha == '♙'){
      ...
      for(var i = 0; i < abecedario.length; i++){
        ...
        var fila = parseInt(posFinal.split(/([a-h])/)[2]) +1;
        // if para mover dos casillas
        if(posInicial == abecedario[i]+7 && posFinal == abecedario[i]+5 && document.getElementById(posFinal).innerText == '' && document.getElementById(abecedario[i]+6).innerText == ''){
          document.getElementById(posFinal).innerHTML = ficha;
          document.getElementById(posInicial).innerHTML = '';
